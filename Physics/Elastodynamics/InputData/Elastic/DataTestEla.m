...
Data.lam_el    = {@(x,y) 1 + 0.*x.*y}; % Data.vp_el^2 * Data.rho_el - 2*Data.mu_el;
%% Forcing terms
% forcing term elastic media
Data.source_f   = {@(x,y,t) pi^2 * 2 * sin(pi*x) .* sin(pi*y)+ 0 * t; 
                   @(x,y,t) pi^2 * 2 * cos(pi*x) .* cos(pi*y) + 0 * t};
...
Data.DirBCEla = {@(x,y,t) sin(pi*x).*sin(pi*y) + 0 * t; 
                 @(x,y,t) cos(pi*x).*cos(pi*y) + 0 * t};
